...
def train_model(model,experiment,train_files,val_files,trackfeat,histfeat,epochs=50,callbacks=None,bit=False,model_name=[None,None]):
    ...
    for epoch in range(epochs):
        ...
        for step,batch in enumerate(setup_pipeline(train_files)):
            ...
            result = model.train_on_batch([batch[z0],batch['trk_feature_Image'],trackFeatures], [batch['pvz0'],batch['trk_fromPV']])   
            ...
            if step%10==0:
                ...
                predictedZ0_NN, predictedAssoc_NN = model.predict_on_batch( [batch[z0],twod_hist,trackFeatures] )
        ...
        for val_step,val_batch in enumerate(setup_pipeline(val_files)):
            ...
            temp_predictedZ0_NN, temp_predictedAssoc_NN  = model.predict_on_batch(
                        [val_batch[z0],val_batch['trk_feature_Image'],val_trackFeatures]
                )
...
def test_model(model,experiment,test_files,trackfeat,histfeat,model_name=[None,None]):
    ...
    for step,batch in enumerate(setup_pipeline(test_files)):
        ...
        predictedZ0_NN_temp, predictedAssoc_NN_temp = model.predict_on_batch( [batch[z0],batch['trk_feature_Image'],trackFeatures] )
...
if __name__=="__main__":
    ...
    network = vtx.nn.E2E2DConv(
            nbins=nbins,
            ntracks=max_ntracks, 
            nhistfeatures=len(histfeat), 
    ...
    with experiment.train():
        train_model(model,experiment,train_files,val_files,trackfeat,histfeat,epochs=epochs,callbacks=reduceLR,bit=bit,model_name=model_name),
    with experiment.test():
        test_model(model,experiment,test_files,trackfeat,histfeat,model_name=model_name)

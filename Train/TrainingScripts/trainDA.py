...
def train_model(model,experiment,train_files,val_files,trackfeat,weightfeat,epochs=50,callbacks=None,nlatent=0,trainable=False):
    ...
    for epoch in range(epochs):
        ...
        for val_step,val_batch in enumerate(setup_pipeline(val_files)):
            ...
            hist,bin_edges = np.histogram(val_batch[z0][0],256,range=(-15,15),weights=val_batch['trk_pt'][0])
            hist = np.convolve(hist,[1,1,1],mode='same')
    
            z0Index = np.argmax(hist)

            hist = tf.convert_to_tensor(hist)
            x_range = tf.range(256, dtype=hist.dtype)

            z0Index=  tf.reduce_sum(tf.nn.softmax(hist)*x_range,axis=-1)


            #print(z0Index)
            #print(temp_predictedZ0_NN[0])
            #print(eval.predictFastHisto(val_batch[z0],val_batch['trk_pt']).flatten()[0])

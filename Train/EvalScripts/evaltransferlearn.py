...
def plotPV_roc(actual,bmNNpred,rmNNpred,FHpred):
    ...
    for i,threshold in enumerate(thresholds):
        ...
        tnbmNN, fpbmNN, fnbmNN, tpbmNN = metrics.confusion_matrix(actual, bmNNpred>threshold).ravel()
        precisionbmNN.append( tpbmNN / (tpbmNN + fpbmNN) )
        recallbmNN.append(tpbmNN / (tpbmNN + fnbmNN) )
        FPRbmNN.append(fpbmNN / (fpbmNN + tnbmNN) )

        tnrmNN, fprmNN, fnrmNN, tprmNN = metrics.confusion_matrix(actual, rmNNpred>threshold).ravel()
        precisionrmNN.append( tprmNN / (tprmNN + fprmNN) )
        recallrmNN.append(tprmNN / (tprmNN + fnrmNN) )
        FPRrmNN.append(fprmNN / (fprmNN + tnrmNN) )
    ...
    ax[1].plot(recallbmNN,FPRbmNN,label=f"Teacher NN AUC: %.4f" %(metrics.roc_auc_score(actual,bmNNpred)))
    ax[1].plot(recallrmNN,FPRrmNN,label=f"Learner NN AUC: %.4f" %(metrics.roc_auc_score(actual,rmNNpred)))
...
if __name__=="__main__":
    ...
    def assocation_mixed_loss(y_true,y_pred):
        Temp = 1
        Kullback_Leibler = tf.keras.losses.KLDivergence()
        KL = Kullback_Leibler(y_true/Temp,y_pred/Temp)
        return KL
    ...
    for step,batch in enumerate(setup_pipeline(test_files)):
        ...
        predictedZ0_rmNN_temp, Z0_bmNN, predictedAssoc_rmNN_temp, Assoc_bmNN, predictedWeights_rmNN, predictedHists_rmNN = redmodel.predict_on_batch(
                        [batch[z0],WeightFeatures,trackFeatures]
                    )

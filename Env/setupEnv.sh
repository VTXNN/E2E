...
function run_setup()
{
    ...
    if [ $(uname) == "Darwin" ]; then
        ...
        curl -sS https://repo.anaconda.com/miniconda/Miniconda3-4.7.12-MacOSX-x86_64.sh -o $INSTALL_ABSDIR/conda.sh || return 1
    elif [ "$(expr substr $(uname -s) 1 5)" == "Linux" ]; then
        echo "Installing for Linux"
        curl -sS https://repo.anaconda.com/miniconda/Miniconda3-4.7.12-Linux-x86_64.sh -o $INSTALL_ABSDIR/conda.sh || return 1
    fi

...
def predictFastHisto(value,weight,eta,eta_weight=False):
    if eta_weight:
        res = res_function(eta)
    else:
        res = np.ones_like(eta)
...
def predictFastHistoMVA(value,weight,eta,MVA,eta_weight=False,cut=True):
    if eta_weight:
        res = res_function(eta)
    else:
        res = np.ones_like(eta)
    ...
    if cut:
        selectCutTracks = (MVA > 0.7)
        weighting = np.ones_like(MVA)
    else:
        selectCutTracks = np.ones_like(weight, dtype=bool)
        weighting = MVA
    
    halfBinWidth = 0.5*30./256.
    hist,bin_edges = np.histogram(value[selectCutTracks],256,range=(-15,15),weights=((weight[selectCutTracks]*weighting[selectCutTracks])/res[selectCutTracks]))
    hist = np.convolve(hist,[1,1,1],mode='same')
    z0Index= np.argmax(hist)
    z0 = -15.+30.*z0Index/256.+halfBinWidth
    return z0

def predictFastHistoChi2(value,weight,eta,bendchi,chirphi,chirz,eta_weight=False):
    if eta_weight:
        res = res_function(eta)
    else:
        res = np.ones_like(eta)

    selectChiTracks = ((bendchi<2.4) & (chirphi < 10) & (chirz < 10))
    
    halfBinWidth = 0.5*30./256.
    hist,bin_edges = np.histogram(value[selectChiTracks],256,range=(-15,15),weights=(weight[selectChiTracks]/res[selectChiTracks]))
    hist = np.convolve(hist,[1,1,1],mode='same')
    z0Index= np.argmax(hist)
    z0 = -15.+30.*z0Index/256.+halfBinWidth
    return z0
...
if __name__=="__main__":
    ...
    for ibatch,data in enumerate(f['L1TrackNtuple']['eventTree'].iterate(branches,entrysteps=5000)):
        ...
        for iev in range(len(data['trk_pt'])):
            ...
            predictedZ0_FH.append(predictFastHisto(data[z0][iev],data['trk_pt'][iev],data['trk_eta'][iev],eta_weight=False))
            predictedZ0_FHz0res.append(predictFastHisto(data[z0][iev],data['trk_pt'][iev],data['trk_eta'][iev],eta_weight=True))
            predictedZ0_FHMVA.append(predictFastHistoMVA(data[z0][iev],data['trk_pt'][iev],data['trk_eta'][iev],data['trk_MVA1'][iev],eta_weight=False,cut=True))
            predictedZ0_FHChi.append(predictFastHistoChi2(data[z0][iev],data['trk_pt'][iev],data['trk_eta'][iev],data['trk_bendchi2'][iev],data['trk_chi2rphi'][iev],data['trk_chi2rz'][iev],eta_weight=False))

            selectPVTracks = (data['trk_fake'][iev]==1)
            predictedZ0_FHNoFakes.append(predictFastHisto(data[z0][iev][selectPVTracks],data['trk_pt'][iev][selectPVTracks],data['trk_eta'][iev][selectPVTracks],eta_weight=False))

            actual_Assoc.append((data['trk_fake'][iev]==1))
            actual_PV.append(data["pv_MC"][iev][0])
            FHassoc = FastHistoAssoc(predictFastHisto(data[z0][iev],data['trk_pt'][iev],data['trk_eta'][iev],eta_weight=False),data[z0][iev],data['trk_eta'][iev])
    ...
    z0_FHChi_array = np.array(predictedZ0_FHChi)
    predictedZ0_FHNoFakes_array = np.array(predictedZ0_FHNoFakes)
    ...
    plt.clf()
    figure,res_width=plotz0_residual((z0_PV_array-predictedZ0_FHChi))
    plt.savefig("%s/Z0ResidualChi.png" % outputFolder)

    plt.clf()
    figure,res_width=plotz0_residual((z0_PV_array-predictedZ0_FHNoFakes))
    plt.savefig("%s/Z0ResidualNoFakes.png" % outputFolder)
    ...
    plt.clf()
    plt.hist(z0_FH_array,range=(-15,15),bins=256,density=True,color='r',histtype="step",label="FastHisto Base")
    plt.hist(z0_FHzres_array,range=(-15,15),bins=256,density=True,color='g',histtype="step",label="FastHisto with z0 res")
    plt.hist(z0_PV_array,range=(-15,15),bins=256,density=True,color='y',histtype="step",label="Truth")
    plt.grid(True)
    ...
    plt.clf()
    plt.hist2d(z0_PV_array, (z0_PV_array-z0_FHChi_array), bins=60,range=((-15,15),(-30,30)) ,norm=matplotlib.colors.LogNorm(vmin=1,vmax=1000))
    plt.xlabel("PV", horizontalalignment='right', x=1.0)
    plt.ylabel("PV - $z_0^{FH}$", horizontalalignment='right', y=1.0)
            #plt.colorbar(vmin=0,vmax=1000)
    plt.tight_layout()
    plt.savefig("%s/ChiFHerr_vs_z0.png" %  outputFolder)

    plt.clf()
    plt.hist2d(z0_PV_array, (z0_PV_array-predictedZ0_FHNoFakes_array), bins=60,range=((-15,15),(-30,30)) ,norm=matplotlib.colors.LogNorm(vmin=1,vmax=1000))
    plt.xlabel("PV", horizontalalignment='right', x=1.0)
    plt.ylabel("PV - $z_0^{FH}$", horizontalalignment='right', y=1.0)
            #plt.colorbar(vmin=0,vmax=1000)
    plt.tight_layout()
    plt.savefig("%s/NoFakesFHerr_vs_z0.png" %  outputFolder)
